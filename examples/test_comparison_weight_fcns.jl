...
function check_majorana_weights(Nmax::Int)
    ...
    for N in 1:Nmax
        ...
        simple_time = @belapsed begin # @btime begin
            for pauli in $paulis
                PauliOperators.simple_majorana_weight(pauli)
            end
        end

        # Time pauli_to_majorana_occupation
        p2m_time = @belapsed begin #@btime begin
            for pauli in $paulis
        ...
        println("  simple_majorana_weight: $(round(simple_time, digits=3)) s")
        println("  pauli_to_majorana_occupation: $(round(p2m_time, digits=3)) s")
    end
    ...
    savefig("SM_vs_P2M_computation_time_qubits_$Nmax.pdf")
    #display(current())
...
mismatches = check_majorana_weights(10)
